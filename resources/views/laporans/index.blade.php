                    ...
                    <div class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
                        <div class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
                            ...
                            <div class="flex flex-wrap items-center gap-2">
                                <a href="{{ route('laporans.show', $laporan) }}" class="rounded-2xl border border-slate-200 bg-slate-50 px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">Detail</a>
                                @if(auth()->user()->isAdmin() || auth()->id() === $laporan->user_id)
                                    <a href="{{ route('laporans.edit', $laporan) }}" class="rounded-2xl border border-slate-200 bg-white px-4 py-2 text-sm font-semibold text-slate-700 hover:bg-slate-100 transition">Edit</a>
